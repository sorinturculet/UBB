...
void testInlocuieste(){
    ...
    for (int i = cMin; i <= cMax; i++) {//se schimba valoarea
        d.inlocuieste(i, i + 1, i + 2);
        assert(d.cauta(i) == i + 2);
    }
    for(int i=cMin-10; i<cMin; i++){//nu exista cheia
        d.inlocuieste(i, i, i+1);
        assert(d.cauta(i) == NULL_TVALOARE);
    }
    for (int i = cMin; i <= cMax; i++) {//nu e buna valoarea veche
        d.inlocuieste(i, i + 100, i + 2);
        assert(d.cauta(i) == i + 2);
    }
}
void testAllExtins() {
	testCreeaza();
    puts("Test creeaza OK");
	testAdauga();
    puts("Test adauga OK");
	testCauta();
    puts("Test cauta OK");
    ...
    puts("Test sterge OK");
	testIterator();
    puts("Test iterator OK");
	testCantitativ();
    puts("Test cantitativ OK");
    testInlocuieste();
    puts("Test inlocuieste OK");
}//problema 15 tema TAD ArboreBinar(cu iterator in preodine,inordine,postordine si latime)- repreentare cu inlantuiri pe tablou
