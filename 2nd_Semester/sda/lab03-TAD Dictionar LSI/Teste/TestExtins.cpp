...
void testAllValues(){
    ...
    assert(valori.size()==20000);
    for (int i = 10000; i > -10000; i--)  { //stergem din containerul vid
        d.sterge(i);
    }
    for (int i = 0; i > -100; i--) { //adaugam mult, si acum prima data adaugam valori mari, dupa aceea mici
        d.adauga(i/2,4);
    }
    vector<TValoare> valori2=d.colecțiaValorilor();
    assert(valori2.size()==50);
    assert(valori2[1]==4);
}
